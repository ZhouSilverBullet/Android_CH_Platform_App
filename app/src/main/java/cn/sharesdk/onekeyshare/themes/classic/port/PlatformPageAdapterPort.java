...
public class PlatformPageAdapterPort extends PlatformPageAdapter {
	...
	protected void calculateSize(Context context, ArrayList<Object> plats) {
		...
		logoHeight = (int) (DESIGN_LOGO_HEIGHT * ratio) + ViewUtil.dp2px(context, 40);
		paddingTop = (int) (DESIGN_PADDING_TOP * ratio) + ViewUtil.dp2px(context, 10);
		bottomHeight = (int) (DESIGN_BOTTOM_HEIGHT * ratio) + ViewUtil.dp2px(context, 10);
		cellHeight = (screenWidth - sepLineWidth * 3) / 5 + ViewUtil.dp2px(context, 60);
		if (plats.size() <= lineSize) {
			panelHeight = cellHeight + sepLineWidth + ViewUtil.dp2px(context, 10);
		} else if (plats.size() <= PAGE_SIZE_P - lineSize) {
			panelHeight = (cellHeight + sepLineWidth) + ViewUtil.dp2px(context, 30);
		} else {
			panelHeight = (cellHeight + sepLineWidth) + ViewUtil.dp2px(context, 30);
		}
	}
