...
public abstract class PlatformPageAdapter extends ViewPagerAdapter implements OnClickListener {
	...
	private View createPanel(Context context) {
		...
		llPanel.setBackgroundColor(0xfff6f6f6);
	...
	public void onClick(View v) {
		String simpleName = v.getTag().getClass().getSimpleName();
		LogUtil.errorLog(getClass().getSimpleName() + "--分享按钮点击" + simpleName);
		...
		if ("qq".equalsIgnoreCase(simpleName)) {
			AnalyticsHome.umOnEvent(AnalyticsHome.QQ_SHARE_CLICK, simpleName);
		} else if ("qzone".equalsIgnoreCase(simpleName)) {
			AnalyticsHome.umOnEvent(AnalyticsHome.QZONE_SHARE_CLICK, simpleName);
		}else if ("sinaweibo".equalsIgnoreCase(simpleName)) {
			AnalyticsHome.umOnEvent(AnalyticsHome.SINAWEIBO_SHARE_CLICK, simpleName);
		}else if ("wechat".equalsIgnoreCase(simpleName)) {
			AnalyticsHome.umOnEvent(AnalyticsHome.WECHAT_SHARE_CLICK, simpleName);
		}else if ("wechatmoments".equalsIgnoreCase(simpleName)) {
			AnalyticsHome.umOnEvent(AnalyticsHome.WECHAT_MOMENTS_SHARE_CLICK, simpleName);
		}
