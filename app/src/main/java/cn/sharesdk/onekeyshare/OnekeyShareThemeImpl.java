...
public abstract class OnekeyShareThemeImpl implements PlatformActionListener, Callback {
	...
	public final boolean handleMessage(Message msg) {
		LogUtil.errorLog("---------handleMessage----------" + msg.arg2);
		if (msg.obj != null) {
			String simpleName = msg.obj.getClass().getSimpleName();
			LogUtil.errorLog("---------handleMessage----------" + simpleName);
			if ("qq".equalsIgnoreCase(simpleName)) {
				AnalyticsHome.umOnEvent(AnalyticsHome.QQ_SHARE_SUCCESS, simpleName);
			} else if ("qzone".equalsIgnoreCase(simpleName)) {
				AnalyticsHome.umOnEvent(AnalyticsHome.QZONE_SHARE_SUCCESS, simpleName);
			}else if ("sinaweibo".equalsIgnoreCase(simpleName)) {
				AnalyticsHome.umOnEvent(AnalyticsHome.SINAWEIBO_SHARE_SUCCESS, simpleName);
			}else if ("wechat".equalsIgnoreCase(simpleName)) {
				AnalyticsHome.umOnEvent(AnalyticsHome.WECHAT_SHARE_SUCCESS, simpleName);
			}else if ("wechatmoments".equalsIgnoreCase(simpleName)) {
				AnalyticsHome.umOnEvent(AnalyticsHome.WECHAT_MOMENTS_SHARE_SUCCESS, simpleName);
			}
		}
		switch (msg.arg1) {
			case 1: {
				...
				LogUtil.errorLog("自定义标签", "类名==OnekeyShareThemeImpl" + "方法名==handleMessage=====:" + "");
			} break;
			case 2: {
				// 失败
				...
				LogUtil.errorLog("自定义标签", "类名==OnekeyShareThemeImpl" + "方法名==handleMessage=====:" + expName);
			} break;
			case 3: {
