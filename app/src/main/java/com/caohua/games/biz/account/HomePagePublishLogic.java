...
public class HomePagePublishLogic extends BaseLogic {
    ...
    public void getData(final String userId, final String requestCount, final String loadedCount, final DataLogicListner listner) {
        ...
        RequestExe.post(HOST_APP + "forum/userArticle", model, new IRequestListener() {
            @Override
            public void success(HashMap<String, String> map) {
                List<HomePagePublishEntry> entry = null;
                if (map != null) {
                    String gameListJson = map.get(HttpConsts.RESULT_PARAMS_DATA);
                    entry = fromJsonArray(gameListJson, HomePagePublishEntry.class);
                }
                if (listner != null) {
