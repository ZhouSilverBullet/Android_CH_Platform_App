...
public class VipActSecondLogic extends BaseLogic {
    public void getActSecond(final DataLogicListner<VipActSecondEntry> listener) {
        ...
        RequestExe.post(HOST_APP + "vip/activeDetail", model, new IRequest2Listener() {
            @Override
            public void success(String result) {
                if (listener != null) {
                    if (!TextUtils.isEmpty(result)) {
                        try {
                            VipActSecondEntry entry = new Gson().fromJson(result, VipActSecondEntry.class);
