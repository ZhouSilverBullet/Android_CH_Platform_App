...
public class DownloadFileInfo extends BaseUrlFileInfo {
    ...
    public static final class Table {
        ...
        public static final String COLUMN_NAME_OF_FIELD_PKG = "pkg";
        public static final String COLUMN_NAME_OF_FIELD_ICON_URL = "icon_url";
        ...
        public static final String getCreateTableSql() {
            ...
            String createTableSql = "CREATE TABLE IF NOT EXISTS " //
                    ...
                    + COLUMN_NAME_OF_FIELD_TITLE + " TEXT,"
                    + COLUMN_NAME_OF_FIELD_PKG + " TEXT,"
                    + COLUMN_NAME_OF_FIELD_ICON_URL + " TEXT,"
    ...
    DownloadFileInfo(DetectUrlFileInfo detectUrlFileInfo) {
        ...
        this.mTitle = detectUrlFileInfo.getTitle();
        this.mPkg = detectUrlFileInfo.getPkg();
        this.mIconUrl = detectUrlFileInfo.getIconUrl();
    ...
    DownloadFileInfo(Cursor cursor) {
        if (cursor != null && !cursor.isClosed()) {
            ...
            columnIndex = cursor.getColumnIndex(Table.COLUMN_NAME_OF_FIELD_PKG);
            if (columnIndex != -1) {
                pkg = cursor.getString(columnIndex);
            }
            columnIndex = cursor.getColumnIndex(Table.COLUMN_NAME_OF_FIELD_ICON_URL);
            if (columnIndex != -1) {
                iconUrl = cursor.getString(columnIndex);
            }
    ...
    void update(DownloadFileInfo downloadFileInfo) {
        ...
        if (!TextUtils.isEmpty(downloadFileInfo.mTitle)) {
            this.mTitle = downloadFileInfo.mTitle;
        }
        if (!TextUtils.isEmpty(downloadFileInfo.mPkg)) {
            this.mPkg = downloadFileInfo.mPkg;
        }
        if (!TextUtils.isEmpty(downloadFileInfo.mIconUrl)) {
            this.mIconUrl = downloadFileInfo.mIconUrl;
        }
    ...
    ContentValues getContentValues() {
        ...
        values.put(Table.COLUMN_NAME_OF_FIELD_TITLE, mTitle);
        values.put(Table.COLUMN_NAME_OF_FIELD_PKG, mPkg);
        values.put(Table.COLUMN_NAME_OF_FIELD_ICON_URL, mIconUrl);
