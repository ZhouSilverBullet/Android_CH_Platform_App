...
public class DownloadTaskManager implements Pauseable {
     ...
     */
    private void addAndRunDetectUrlFileTask(DownloadParams params, boolean forceDetect, OnDetectBigUrlFileListener
        ...
        DetectUrlFileTask detectUrlFileTask = new DetectUrlFileTask(params, mConfiguration.getFileDownloadDir(),
     ...
     */
    private void detectInternal(DownloadParams params, boolean forceDetect, OnDetectBigUrlFileListener
            onDetectBigUrlFileListener, DownloadConfiguration downloadConfiguration) {
        // start detect task
        addAndRunDetectUrlFileTask(params, forceDetect, onDetectBigUrlFileListener, downloadConfiguration);
    }

    /**
     * detect a url file
     *
     * @param params                     file url
     * @param onDetectUrlFileListener OnDetectUrlFileListener impl
     * @param downloadConfiguration   download configuration
     * @deprecated this method can not detect the url file which bigger than 2G, use {@link #detect(DownloadParams,
     * OnDetectBigUrlFileListener, DownloadConfiguration)}instead
     */
    @Deprecated
    public void detect(DownloadParams params, final OnDetectUrlFileListener onDetectUrlFileListener, DownloadConfiguration
            downloadConfiguration) {
        final String finalUrl = params.url;
        detect(params, new OnDetectBigUrlFileListener() {// inner change callback
            @Override
     ...
     * @param downloadConfiguration      download configuration
     */
    public void detect(DownloadParams params, OnDetectBigUrlFileListener onDetectBigUrlFileListener, DownloadConfiguration
            downloadConfiguration) {
        detectInternal(params, false, onDetectBigUrlFileListener, downloadConfiguration);
    }

    // --------------------------------------create/continue downloads--------------------------------------

    /**
     * create a new download after detected a url file by using {@link #detect(DownloadParams, OnDetectBigUrlFileListener,
     * DownloadConfiguration)}
     * <br/>
     ...
     * @param downloadConfiguration download configuration
     */
    public void start(final DownloadParams params, final DownloadConfiguration downloadConfiguration) {
        String url = params.url;
        DownloadFileInfo downloadFileInfo = getDownloadFile(url);
        // has been downloaded
        ...
        // not download
            ...
            // not detect
                ...
                final String finalUrl = url;
                // detect first
                detect(params, new OnDetectBigUrlFileListener() {
                    @Override
                    public void onDetectUrlFileFailed(String url, DetectBigUrlFileFailReason failReason) {
                        DetectUrlFileInfo detectUrlFileInfo = getDetectUrlFile(url);
                        if (detectUrlFileInfo == null) {
                            DownloadFileInfo file = mDownloadRecorder.getDownloadFile(url);
                            if (file == null) {
                                detectUrlFileInfo = new DetectUrlFileInfo(params.url, 1048576, null, "",
                                        "bytes", mConfiguration.getFileDownloadDir(), params.title + params.url.hashCode() + ".apk", params.title, params.pkg, params.iconUrl);
                                // add or update to memory cache
                                mDetectUrlFileCacher.addOrUpdateDetectUrlFile(detectUrlFileInfo);
                                file = mDownloadRecorder.getDownloadFile(url);
                                mDownloadRecorder.createDownloadFileInfo(detectUrlFileInfo);
                            }
                        }
     ...
     * @param downloadConfiguration download configuration
     */
    public void start(List<DownloadParams> urls, DownloadConfiguration downloadConfiguration) {
        for (DownloadParams url : urls) {
     ...
     * restart a download
     */
    private void reStartInternal(final DownloadParams params, final boolean isDelete, final DownloadConfiguration
            downloadConfiguration) {
        ...
        if (DownloadFileUtil.isLegal(downloadFileInfo)) {
            ...
            // 2.detect
            detectInternal(params, true, new OnDetectBigUrlFileListener() {
                @Override
                public void onDetectNewDownloadFile(String url, String fileName, String saveDir, long fileSize) {
                    try {
                        // delete old one
                ...
                public void onDetectUrlFileExist(String url) {
                    try {
                        ...
                        mDownloadRecorder.resetDownloadFile(finalUrl, isDelete);
                        // start download
                        start(params, downloadConfiguration);
                    } catch (Exception e) {
                        e.printStackTrace();
                        // TODO need a special exception ?
                        FileDownloadStatusFailReason failReason = new OnFileDownloadStatusFailReason(finalUrl, e);
     ...
     * @param downloadConfiguration download configuration
     */
    public void reStart(final DownloadParams params, final DownloadConfiguration downloadConfiguration) {
        ...
        if (isDownloading(url)) {
            ...
            pause(url, new OnStopFileDownloadTaskListener() {
                ...
                public void onStopFileDownloadTaskSucceed(String url) {
                    ...
                    reStartInternal(params, true, downloadConfiguration);
                }
     ...
     * @param downloadConfiguration download configuration
     */
    public void reStart(List<DownloadParams> urls, DownloadConfiguration downloadConfiguration) {
        for (DownloadParams url : urls) {
